...
idlecode: idlecode.S
	...
	$(LD) $(LDFLAGS) -N -e start -Ttext 0x0 -o idlecode.out idlecode.o
...
clean: 
	...
	*.o *.d *.asm *.sym vectors.S parport.out bootother.out bootother \
	bootblock kernel xv6.img fs.img mkfs *~ idlecode idlecode.out initcode initcode.out \
	$(UPROGS)
...
smp:  fs.img xv6.img
	qemu -parallel stdio -hdb fs.img xv6.img -smp 4